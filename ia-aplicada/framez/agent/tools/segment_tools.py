...
@tool
def decide_segment(analysis: str, duration: float) -> dict:
    """
    Segments a video into smaller clips based on the video's content.

    Call after extract_frames.
    Input: video_path.
    Returns: a list of segments with their start and end times.
    """
    try:
        ...
        response = client.llm_router(
            prompt=decide_prompt(duration, analysis),
        ...
        print(f"Segmentos decididos: {segments}")
