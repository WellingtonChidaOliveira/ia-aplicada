...
from agent.nodes.analyseFrame import (
    load_base64,
    analyse_frame,
    analyse_frames,
    UNAVAILABLE_LABEL,
...
class TestAnalyseFrame(unittest.TestCase):
    @patch("agent.nodes.analyseFrame.time.sleep")
    @patch("agent.nodes.analyseFrame.send_image_ollama")
    @patch("agent.nodes.analyseFrame.load_base64", return_value="base64data")
    ...
    @patch("agent.nodes.analyseFrame.send_image_ollama")
    @patch("agent.nodes.analyseFrame.load_base64", return_value="base64data")
    def test_parses_timestamp_from_frame_name(
        self, _mock_b64, mock_ollama, mock_sleep
    ):
        mock_response = MagicMock()
...
class TestAnalyseFrames(unittest.TestCase):
    @patch("agent.nodes.analyseFrame.time.sleep")
    @patch(
        "agent.nodes.analyseFrame.analyse_frame",
    ...
    def test_frames_are_sorted(self, mock_analyse, mock_sleep):
        ...
        state = mock_state(
            frames=["frame_0003_t018.75.jpg", "frame_0001_t006.25.jpg"]
        )
    ...
    @patch("agent.nodes.analyseFrame.time.sleep")
    @patch("agent.nodes.analyseFrame.analyse_frame", return_value=UNAVAILABLE_LABEL)
    ...
    @patch("agent.nodes.analyseFrame.time.sleep")
    @patch("agent.nodes.analyseFrame.analyse_frame")
